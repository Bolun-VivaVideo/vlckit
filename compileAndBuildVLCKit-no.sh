...
TESTEDHASH="4cbd406c" # libvlc hash that this version of VLCKit is build on
...
if [ "$TVOS" = "yes" ];then
    ...
    if [ "$FARCH" = "all" ] || (! is_simulator_arch $FARCH);then
        ...
        frameworks="$frameworks -framework VLCKit-${platform}.xcarchive/Products/Library/Frameworks/VLCKit.framework"
    fi
...
if [ "$MACOS" = "yes" ];then
    ...
    rm -rf macOS
    mkdir macOS
    xcodebuild -create-xcframework -framework VLCKit-macosx.xcarchive/Products/Library/Frameworks/VLCKit.framework -output macOS/VLCKit.xcframework
    spopd # build
